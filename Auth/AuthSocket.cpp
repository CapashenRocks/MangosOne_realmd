...
bool AuthSocket::_HandleLogonProof()
{
    ...
    if (!memcmp(M.AsByteArray(), lp.M1, 20))
    {
        ...
        // Use direct execute so the new sessionkey is immediately visible to mangosd during world auth.
          /* This fixed an issue where the correct login could disconnect and give "WorldSocket::HandleAuthSession: Sent Auth Response (authentification failed)" message
            due to intermittent world auth failures caused by a "stale" account.sessionkey writes in realmd.
            The fix changes the account session key update from queued PExecute to synchronous DirectPExecute so the world server no longer reads the previous session key during fresh client logins.*/
        LoginDatabase.DirectPExecute(
            "UPDATE `account` "
            "SET `sessionkey` = '%s', `last_ip` = '%s', `last_login` = NOW(), `locale` = '%u', `os` = '%s', `failed_logins` = 0 "
            "WHERE `username` = '%s'",
             K_hex,
